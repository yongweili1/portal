...
export class AuthHttp {
  ...
  public requestWithToken(req: Request, token: string): Observable<Response> {
    // 截取的请求的 uri 地址
    const uri = req.url.substring(req.url.indexOf('/api'));
    // 放行的白名单
    const whiteUrls = ['/api/verify-code', '/api/verify-code/*', '/api/forgot-password',
      '/api/forgot-password/*', '/api/forgot-password/**'];
    for (const url of whiteUrls) {
      const str_reg = url.replace('**', '[A-Za-z0-9_\\-\\./]+').replace('*', '[A-Za-z0-9_\\-]+').replace('?', '\\w{1}');
      const regex = new RegExp(str_reg);
      if (regex.test(uri)) {
        // 直接发送 http 请求 
