...
///
///  \file      uai_app_cryptohash.h
///  \brief     calc MD5 hash
///             Usage
///             
///             std::string text("firstname=zhangsan");
///             md5_helper_t hhelper;
///             std::string hash = hhelper.hexdigesttext(text);
...
#ifndef UAI_APP_CRYPTOHASH_H_
#define UAI_APP_CRYPTOHASH_H_
...
public:
    ...
    cryptohash_t(void) :
    m_hCryptProv(NULL), m_hHash(NULL)
    {
...
public:
    ...
    std::string hexdigesttext(
        const std::string& text, bool uppercase = false)
    {
