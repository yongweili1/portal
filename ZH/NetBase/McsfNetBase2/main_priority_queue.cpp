...
int main()
{
    ...
    acceptor.async_accept(server_socket,
        pri_queue.wrap(0, high_priority_handler));
    ...
    timer.async_wait(pri_queue.wrap(0, middle_priority_handler));

    while (size_t t = io_service.run_one())
    {
        printf("%d\n",t);
        // The custom invocation hook adds the handlers to the priority queue
        // rather than executing them from within the poll_one() call.
        while (t = io_service.poll_one())
            printf("t=%d\n",t);
